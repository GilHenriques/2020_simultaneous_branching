...
data_files <- dir(path = "./2020-08 IBS asymmetry/data")
...
for(i in 1:length(data_files)){
  ...
  if (file %notin% previous_files) {
    df <- fread(paste0("./2020-08 IBS asymmetry/data/", file), col.names = c("t", "x", "y")) %>%   
    ...
    angles[[i]]$alpha <- (str_split(file, pattern = "alpha")[[1]][2] %>% str_split(pattern = ".txt"))[[1]][1]
    print(i)
  }
}

saveRDS(angles, file = "2020-08 IBS asymmetry/angles_rds")
