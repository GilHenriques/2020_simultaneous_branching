...
data_files <- dir(path = "./2020-07 IBS/data2")
...
angles <- readRDS("2020-07 IBS/angles_rds") # read in angles that have already been calculated since before
...
for(i in 1:length(data_files)){
   ...
   if (file %notin% previous_files) {
      df <- fread(paste0("./2020-07 IBS/data2/", file), col.names = c("t", "x", "y")) %>%   
...
saveRDS(angles, file = "2020-07 IBS/angles_rds")
...
plot(angle_onset, angle_branching)
