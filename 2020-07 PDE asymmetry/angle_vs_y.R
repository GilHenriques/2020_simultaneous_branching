...
directories <- dir(path = "2020-07 PDE asymmetry/data/", pattern = "2020-03-27")
threshold <- 1e-6
...
for(directory in directories){
  timepoints <- dir(path = paste0("2020-07 PDE asymmetry/data/", directory), pattern = "full_population")
  timepoints <- gtools::mixedsort(timepoints)
  maxXY <- NA
  
  for (timepoint in timepoints){
    df <- readRDS(paste("2020-07 PDE asymmetry/data", directory, timepoint, sep = "/"))[[1]]
    if (multimodal(df)) { 
...
saveRDS(angles_df, "2020-07 PDE asymmetry/angle_vs_y_rds")
...
angles_df %>% 
  ...
  scale_y_continuous(limits = c(pi/2, pi),
                     ...
                     labels = c( expression(frac(pi, 2)),
                                 ...
                                 expression(pi)) ) +
  xlab("Asymmetry between traits")

ggsave("angle_vs_y.pdf")
