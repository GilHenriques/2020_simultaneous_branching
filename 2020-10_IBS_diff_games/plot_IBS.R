...
rds_files <- list.files("2020-10_IBS_diff_games/", pattern = "rds")
angles_approach <- parse_number(rds_files)

angles <- vector(mode = "list", length = length(rds_files))
for (i in seq_along(rds_files)) {
  angles[[i]] <- readRDS(paste0("2020-10_IBS_diff_games/", rds_files[i])) %>% 
...
plot_ibs <- plot_angles %>% 
  ...
  geom_segment(data = angles_origin,
               aes(x = origin_angle, xend = origin_angle, y = 0.2, yend = 0.02),
...
saveRDS(plot_ibs, file = "./2020-10_IBS_diff_games/plot_ibs_rds")
