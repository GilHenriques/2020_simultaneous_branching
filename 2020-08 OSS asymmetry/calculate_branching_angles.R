...
output_from_OSS <- dir(path = "./2020-08 OSS asymmetry/", pattern = "output_list")
asymmetry <- parse_number(output_from_OSS)

branching_angles_list <- vector(mode = "list", length = length(asymmetry))

for(i in seq_along(output_from_OSS)){
  data <- paste0("./2020-08 OSS asymmetry/", output_from_OSS[i]) %>% 
    read_rds() %>% 
    map(. %>% drop_na())
  
  branching_angles_df <- tibble(asymmetry = asymmetry[i], rep = seq_along(data), angle_of_branching = 999)
...
saveRDS(all_branching_angles, file = "./2020-08 OSS asymmetry/branching_angles_rds")
